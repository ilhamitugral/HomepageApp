    ...
    if($email == "") {
        ...
        if(!$uppercase || !$lowercase || !$number || strlen($password) < 8) {
            ...
            if($count > 0) {
                ...
                $query = mysqli_query($db, "INSERT INTO users SET ip = '$ip', date = '$date', register_date = '$date', lang='".USER_LANG."', token = '$token', email = '$email', username = '$username', name = '', surname = '', password = '$password', is_active = 0, theme = 'default', design = '', settings = ''");
                $error = $db->error;
                if($query) {
                    mysqli_commit($db);
                    $_SESSION["login"] = true;
